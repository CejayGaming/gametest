...
switch(message_id){
	...
	case 2:														//New Connection
		var newSocket = buffer_read(buffer,buffer_u8);
		global.newIp = buffer_read(buffer,buffer_string);
		ds_list_add(global.socketList,newSocket);				
		ds_list_add(global.ipList,global.newIp);
		instance_create_layer(0,0,"Instances",obj_player2);
		break;
	case 3:														//Disconnection
		ds_list_delete(global.socketList,ds_list_find_index(global.socketList,buffer_read(buffer,buffer_u8)));
		ds_list_delete(global.ipList,ds_list_find_index(global.ipList,buffer_read(buffer,buffer_string)));
		break;
}